...
export default function Register() {
  ...
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (_event, session) => {
      if (session?.user?.email) {
        const { data } = await supabase.from("students").select("*").eq("email", session.user.email).maybeSingle();
        if (!data) {
          await supabase.from("students").insert({
            full_name: session.user.user_metadata?.full_name || "Aluno(a)",
            email: session.user.email,
            status: "active"
          });
          toast({ title: "Conta criada com sucesso!", description: "Bem-vindo à Cia fitness!" });
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Conta criada com sucesso!", description: "Você já faz parte da Cia fitness!" });
  ...
  return (
    <div className="max-w-md mx-auto mt-10 p-6 bg-card border border-border rounded-xl shadow-sm">
      ...
      <Button type="button" variant="outline" onClick={handleGoogleLogin} className="w-full gap-2 mb-4">
      ...
      <div className="mt-4 text-center"><Link to="/student-area" className="text-sm text-muted-foreground hover:text-primary">Já tenho conta? Acessar Área do Aluno</Link></div>
    </div>
  );
}